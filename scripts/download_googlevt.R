...
download_googlevt<-function(directory,
                          maintenance_file,
                          gdrive = F,
                          gshared_drive,
                          output_file){
  ...
  if(gdrive==T){
   ...
   # drive_auth(cache = Sys.getenv('GDRIVE_PAT'), email = 'abreefpilz@vt.edu')
    googledrive::drive_auth(cache = '.secrets', email = 'abreefpilz@vt.edu')
